...
for (let i = 0; i < toolBoxColors.length; i++) {
    ...
    toolBoxColors[4].addEventListener("click",function(){
        let allTickets = document.querySelectorAll(".ticket-cont");
        for (let j = 0; j < allTickets.length; j++) {
            allTickets[j].remove();
        }
...
for (let i = 0; i < toolBoxColors.length; i++) {
    let priorityDivOneColor = toolBoxColors[i];
    priorityDivOneColor.addEventListener("click", function (e) {
        for (let j = 0; j < toolBoxColors.length; j++) {
            toolBoxColors[j].classList.remove("view");
        }
        priorityDivOneColor.classList.add("view");
        modalPriorityColor = priorityDivOneColor.classList[0];
    })
}
